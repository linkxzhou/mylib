...
#include "./base/select_dispatcher.h"
#include <sys/select.h>
#include <algorithm>

// Select服务器 - 使用select进行I/O多路复用
class SelectServer : public ServerBase {
public:
    SelectServer(int port = 8080) : ServerBase(port) {
        dispatcher_ = std::unique_ptr<EventDispatcher>(new SelectDispatcher(1024));
    }
    ...
    bool start() override {
        ...
        running_ = true;
        std::cout << "Select Server started on port " << port_ << std::endl;
...
private:
    void handle_new_connection() {
        ...
        if (!dispatcher_->add_event(client_fd, EventType::READ | EventType::ERROR, client_callback)) {
