...
jsval_t mkval(uint8_t type, uint64_t data) { return ((jsval_t) 0x7ff0U << 48U) | ((jsval_t) (type) << 48) | (data & 0xffffffffffffUL); }
...
static void js_fixup_offsets(struct js *js, jsoff_t start, jsoff_t size)
{
    ...
    { // start from 0!
        ...
        if ((v & 3) == T_OBJ)
        {
            ...
            if (u > start)
                saveoff(js, (jsoff_t)(off + sizeof(jsoff_t)), u - size);
        }
        if ((v & 3) == T_PROP)
        {
            jsoff_t koff = loadoff(js, (jsoff_t)(off + sizeof(off)));
            if (koff > start)
                saveoff(js, (jsoff_t)(off + sizeof(off)), koff - size);
...
static jsoff_t js_unmark_entity(struct js *js, jsoff_t off)
{
    ...
    if (v & GCMASK)
    {
        ...
        // printf("UNMARK %5u %d\n", off, v & 3);
        if ((v & 3) == T_OBJ)
            js_unmark_entity(js, v & ~(GCMASK | 3));
        if ((v & 3) == T_PROP)
        {
            ...
            if (is_mem_entity(vtype(val)))
                js_unmark_entity(js, (jsoff_t)vdata(val));
        }
    }
    return v & ~(GCMASK | 3U);
}
...
void js_gc(struct js *js)
{
    // printf("================== GC %u\n", js->nogc);
    setlwm(js);
    if (js->nogc == (jsoff_t)~0)
        return; // ~0 is a special case: GC Is disabled
...
static jsval_t do_dot_op(struct js *js, jsval_t l, jsval_t r)
{
    ...
    if (vtype(r) != T_CODEREF)
        return js_mkerr(js, "ident expected");
...
static jsval_t call_js(struct js *js, const char *fn, jsoff_t fnlen)
{
    ...
    if (!is_err(res) && !(js->flags & F_RETURN))
        res = js_mkundef(); // No return
...
static jsval_t js_obj_literal(struct js *js)
{
    ...
    while (next(js) != TOK_RBRACE)
    {
        ...
        {
            if (exe)
                key = js_mkstr(js, js->code + js->toff, js->tlen);
        }
        else if (js->tok == TOK_STRING)
        {
            if (exe)
                key = js_str_literal(js);
        }
        else
        {
...
static jsval_t js_literal(struct js *js)
{
    ...
    setlwm(js);
    // printf("css : %u\n", js->css);
    if (js->maxcss > 0 && js->css > js->maxcss)
        return js_mkerr(js, "C stack");
    js->consumed = 1;
    switch (js->tok)
    { // clang-format off
...
static jsval_t js_logical_and(struct js *js)
{
    ...
    while (next(js) == TOK_LAND)
    {
        ...
        if (!js_truthy(js, res))
            js->flags |= F_NOEXEC; // false && ... shortcut
        if (js->flags & F_NOEXEC)
        {
            js_logical_and(js);
        }
...
static jsval_t js_logical_or(struct js *js)
{
    ...
    while (next(js) == TOK_LOR)
    {
        ...
        res = resolveprop(js, res);
        if (js_truthy(js, res))
            js->flags |= F_NOEXEC; // true || ... shortcut
        if (js->flags & F_NOEXEC)
        {
            js_logical_or(js);
        }
...
static jsval_t js_let(struct js *js)
{
    ...
    for (;;)
    {
        ...
        if (next(js) == TOK_ASSIGN)
        {
            ...
            v = js_expr(js);
            if (is_err(v))
                return v; // Propagate error if any
        }
        if (exe)
        {
        ...
        if (next(js) == TOK_SEMICOLON || next(js) == TOK_EOF)
            break; // Stop
        EXPECT(TOK_COMMA, );
    }
    return js_mkundef();
...
static jsval_t js_for(struct js *js)
{
    ...
    if (!expect(js, TOK_SEMICOLON, &res))
        goto done;
    js->flags |= F_NOEXEC;
    ...
    while (!(flags & F_NOEXEC))
    {
        ...
        if (is_err2(&v, &res))
            goto done; // Fail on error
        if (js->flags & F_BREAK)
            break;                                           // break was executed - exit the loop!
        js->flags = flags, js->pos = pos2, js->consumed = 1; // Jump to final expr
        if (next(js) != TOK_RPAREN)
        {                    // Is it specified?
            v = js_expr(js); // Yes. Execute it
            if (is_err2(&v, &res))
                goto done; // On error, fail short
        }
    }
...
static jsval_t js_return(struct js *js)
{
    ...
    if (exe && !(js->flags & F_CALL))
        return js_mkerr(js, "not in func");
    if (next(js) == TOK_SEMICOLON)
        return js_mkundef();
