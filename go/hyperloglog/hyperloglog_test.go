...
func TestHLL_EstimateV1(t *testing.T) {
	for _, sz := range []int{10000000} {
		t.Run(fmt.Sprintf("size=%d", sz), func(t *testing.T) {
			data := make([]uint64, sz)
			for i := range data {
				data[i] = uint64(rand.Int63())
			...
			sk1 := New14()
			for _, v := range data {
				sk1.Insert([]byte(fmt.Sprintf("key%d%dtest", v, v)))
			}
			...
			sk1Estimate := sk1.Estimate()
			sk2Estimate := sk2.Estimate()
			fmt.Printf("sk1.Estimate() = %v, sk2.Estimate() = %v, ratio: %v\n", sk1Estimate, sk2Estimate, float64(sk1Estimate) / float64(sz))
		})
	}
}

func BenchmarkHLL_EstimateV1(b *testing.B) {
	for i := 0; i < b.N; i++ {
		sz := 10000000
		data := make([]uint64, sz)
		for i := range data {
		...
		sk1Estimate := sk1.Estimate()
		fmt.Printf("sk1.Estimate() = %v, ratio: %v\n", sk1Estimate, float64(sk1Estimate) / float64(sz))
	}
}
